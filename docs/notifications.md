...
## Order Confirmation Email Renderer

Order confirmation content is generated by the pure renderer at `lib/notifications/order-confirmation.ts`. The renderer accepts validated order details and the existing ticket cart shape, then returns a subject, preview text, HTML body, and plain-text body. It does not create a Resend client, send email, read environment variables, persist data, or mark an order as notified.

The confirmation includes guest-facing order details, pass-level package summaries, selected shirt and koozie options, included sticker notes, order totals, and a simple reminder that Bubelpalooza merch is picked up at check-in on event day. Event copy is limited to the verified public facts in `docs/theme.md`: Bubelpalooza at Bubel Beach Club in Leander, TX on Sunday, May 24, 2026, with the pool opening at 12 PM, the boil hitting the table at 1 PM, live music kicking off at 2:30 PM, and the confirmed food menu.

The intended send path is:

- A payment-confirmed server path, such as a verified Stripe webhook, loads the saved order and recipient details.
- The caller validates or reconstructs the cart data with the ticket cart schema before rendering.
- The caller passes only the public order reference, purchaser name or email, pass selections, merch selections, donation amount, and total needed for the confirmation.
- The caller sends the returned subject, HTML, and text through Resend after checking idempotency so a confirmed order receives one confirmation email.
- Send failures are logged as coarse operational events without raw provider payloads, secrets, checkout session bodies, or full personal data.

Until that delivery path exists, the renderer can be used for private previews and tests only. Do not imply in guest-facing UI that the application has sent a confirmation email unless the send path has actually completed.
